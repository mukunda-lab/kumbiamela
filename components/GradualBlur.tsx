...
const PRESETS: Record<string, Partial<ResolvedConfig>> = {
...
const mergeConfigs = (
  ...configs: Array<Partial<ResolvedConfig>>
): ResolvedConfig =>
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const dynamic = config as unknown as Record<string, string | undefined>;
      const suffix = key[0].toUpperCase() + key.slice(1);
      let v = config[key];
      if (w <= 480 && dynamic[`mobile${suffix}`]) v = dynamic[`mobile${suffix}`];
      else if (w <= 768 && dynamic[`tablet${suffix}`])
        v = dynamic[`tablet${suffix}`];
      else if (w <= 1024 && dynamic[`desktop${suffix}`])
        v = dynamic[`desktop${suffix}`];
      setValue(v);
    };
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const containerStyle = useMemo<CSSProperties>(() => {
    ...
    const baseStyle: CSSProperties & Record<string, unknown> = {
